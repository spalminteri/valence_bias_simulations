...
% Model simulator, for two-armed bandit task, with safe vs. risky
...
rewardprob=0.5;                         % probability of the risky option 
...
for n=1:nsubjects;
    ...
    [choicesRe(n,:),  outcomesRe(n,:), probaRe(n,:)  Q1Re(n,:)  Q2Re(n,:)]  = RiskTask(paramsub,trials,rewardprob,initialization,rescale);
    [choicesRe2(n,:), outcomesRe2(n,:),probaRe2(n,:) Q1Re2(n,:) Q2Re2(n,:)] = RiskTask(paramsub2,trials,rewardprob,initialization,rescale);
    [choicesRe3(n,:), outcomesRe3(n,:),probaRe3(n,:) Q1Re3(n,:) Q2Re3(n,:)] = RiskTask(paramsub3,trials,rewardprob,initialization,rescale);
    

end


%% Plot the choice rate 
figure;
...
%title('"" task')
ylabel('"R" choice rate')
xlabel('Trials')
set(gca,'Fontsize',24)
legend('\alpha_+ = \alpha_-','\alpha_+ > \alpha_-','\alpha_+ < \alpha_-','Location','northwest')
